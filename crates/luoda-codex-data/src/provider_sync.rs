...
pub fn run_provider_sync_with_target(
    ...
    let sync_result = (|| -> anyhow::Result<ProviderSyncResult> {
        ...
        let cwd_by_thread_id = collected
            ...
            .collect::<HashMap<_, _>>();
        let sqlite_paths = luoda_codex_core::codex_sqlite::codex_session_db_paths_from_home(&home);
...
pub fn load_provider_sync_targets(codex_home: Option<&Path>) -> ProviderSyncTargetList {
    ...
    for db_path in luoda_codex_core::codex_sqlite::codex_session_db_paths_from_home(&home) {
...
fn build_encrypted_content_warning(
    ...
    Some(format!(
        "检测到 {total} 个会话文件包含来�?{} �?encrypted_content。可见会话元数据已同步到 {target_provider}，但继续或压缩这些历史可能出�?invalid_encrypted_content；需要可靠续聊时请切回原供应�?账号或开启新会话�?,
...
fn create_backup(
    ...
    for db_path in luoda_codex_core::codex_sqlite::codex_session_db_paths_from_home(home) {
        for source in luoda_codex_core::codex_sqlite::codex_sqlite_sidecar_paths(&db_path) {
            if !source.exists() {
                continue;
            }
            let relative = luoda_codex_core::codex_sqlite::relative_to_codex_home(home, &source);
    ...
    fs::write(
        ...
        serde_json::to_string_pretty(&json!({
            ...
            "managedBy": "Luoda-Codex provider sync"
        }))?,
    )?;
    Ok(backup_dir)
}
...
fn prune_backups(home: &Path) -> anyhow::Result<()> {
    ...
    for entry in fs::read_dir(&root)? {
        ...
        if value.get("managedBy").and_then(Value::as_str) == Some("Luoda-Codex provider sync") {
            managed.push(path);
        }
    }
