use luoda_codex_core::models::{ExportResult, ExportStatus, SessionRef};
...
impl MarkdownExportService {
    ...
    pub fn export(&self, session: &SessionRef) -> ExportResult {
        let Some(db_path) = &self.db_path else {
            return failed(&session.session_id, "未配置本�?Codex 数据�?);
        };
        if !db_path.exists() {
            return failed(
                &session.session_id,
        ...
        let result = (|| -> anyhow::Result<ExportResult> {
            ...
            if !supports_codex_threads(&db)? {
                return Ok(failed(&thread_id, "不支持当前本地存储结�?));
            }
            ...
            let (_, title, rollout_path) = match row {
                ...
                Err(rusqlite::Error::QueryReturnedNoRows) => {
                    return Ok(failed(&thread_id, "未找到对应会�?));
                }
