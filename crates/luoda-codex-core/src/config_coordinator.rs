...
pub fn detect_external_modification(home: &std::path::Path) -> Option<String> {
    ...
    if writer == WRITER_CODEX_PLUS_PLUS {
        Some(
            "检测到 config.toml / auth.json 在 Luoda-Codex 上次写入后被外部修改。\
        ...
        Some(
            "检测到 live 配置与 CC Switch 上次写入记录不一致。\
             请先在 CC Switch 中重新启用当前供应商，或刷新 Luoda-Codex 联动状态。"
                .to_string(),
        )
    } else {
        Some(
            "检测到 config.toml / auth.json 与 Luoda-Codex 记录不一致，\
             可能已被其他工具改写。"
                .to_string(),
        )
    }
...
pub fn evaluate_live_write(settings: &BackendSettings, force: bool) -> LiveConfigWriteDecision {
    if !settings.relay_profiles_enabled {
        return LiveConfigWriteDecision {
            allowed: false,
            message: "供应商配置总开关已关闭，Luoda-Codex 不会写入 config.toml / auth.json。".to_string(),
        };
    }
    ...
    if effective == ConfigOwnership::CcSwitch && !force {
        return LiveConfigWriteDecision {
            ...
            message: "当前由 CC Switch 管理 Codex 供应商配置。\
                      Luoda-Codex 不会直接覆盖 live 配置；请通过联动供应商切换，或在 CC Switch 中切换后刷新。"
                .to_string(),
        };
    }
        ...
        && let Some(conflict) = detect_external_modification(&relay_config::default_codex_home_dir())
    {
        return LiveConfigWriteDecision {
            allowed: false,
            message: format!(
                "{conflict} 若确认由 Luoda-Codex 接管，请先将“配置所有权”设为 Luoda-Codex 并强制切换。"
            ),
        };
    }
...
fn coordination_guidance(
    ...
    if !ccswitch_detected {
        return "未检测到 CC Switch，Luoda-Codex 将独立管理 ~/.codex/config.toml。".to_string();
    }
    ...
    match effective {
        ConfigOwnership::CcSwitch => {
            let provider = current_ccs
                ...
                .unwrap_or("未知");
            format!(
                "当前由 CC Switch 管理供应商配置。Luoda-Codex 会通过联动读取/回写 cc-switch 数据库，\
                 不会直接覆盖 live 配置。CC Switch 当前供应商：{provider}；live model_provider：{live_model_provider}。"
            )
        }
        ConfigOwnership::CodexPlusPlus => {
            if settings.ccs_link_enabled {
                "已开启 cc-switch 联动，但配置所有权仍为 Luoda-Codex。切换供应商时 Luoda-Codex 会写入 live 配置，\
                 可能覆盖 CC Switch 的选择。"
                    .to_string()
            } else {
                "Luoda-Codex 独立管理 live 配置。若同时使用 CC Switch，建议开启联动并设为 auto/ccswitch 所有权。"
                    .to_string()
            }
        }
