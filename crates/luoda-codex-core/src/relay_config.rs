...
const CHAT_UPSTREAM_BASE_URL_KEY: &str = "luoda_codex_chat_base_url";
...
fn table_mut_or_insert<'a>(doc: &'a mut DocumentMut, key: &str) -> anyhow::Result<&'a mut Table> {
    ...
    doc.get_mut(key)
        ...
        .ok_or_else(|| anyhow::anyhow!("{key} 必须�?TOML table"))
}
...
pub fn chatgpt_auth_status_from_home(home: &Path) -> ChatGptAuthStatus {
    ...
    if let Some(account_label) = auth_json_chatgpt_account_label(&auth_path) {
        return ChatGptAuthStatus {
            ...
            account_label,
            message: "已通过 auth.json �?config.toml 检测到 ChatGPT 登录�?.to_string(),
        };
    }
...
pub async fn test_relay_profile(
    ...
    // 如果 404 �?base_url 末尾没有 /v1，尝试自动补 /v1 后再发一次�?
    // 许多上游（中转站、自建代理）暴露的路径以 /v1/ 开头，
    // 用户容易遗漏这个前缀，导�?/responses �?/chat/completions 404�?
...
pub fn upsert_context_entry_in_common_config(
    ...
    if doc[table_name].as_table().is_none() {
        anyhow::bail!("{table_name} 必须�?TOML �?);
    }
...
fn apply_context_limits_to_config(
    ...
    let mut doc = parse_toml_document(config_text)?;
    if let Some(value) = parse_optional_positive_u64(context_window, "上下文大�?)? {
        doc["model_context_window"] = toml_edit::value(value as i64);
    }
    if let Some(value) = parse_optional_positive_u64(auto_compact_limit, "压缩上下文大�?)? {
...
fn restore_profile_auth_from_live_config(
    ...
    if let Some(auth_object) = auth.as_object_mut() {
        ...
        anyhow::bail!("auth.json 必须�?JSON 对象");
    }
...
/// 解析 profile 實際使用的模型：優先�?config.toml 裡的 `model =`�?
/// 否則退�?profile.model 欄位。供應商測試用它做回退，避免串到別家供應商的模型名�?
...
fn ensure_provider_table<'a>(
    ...
    providers
        .get_mut(provider_id)
        .and_then(Item::as_table_mut)
        .ok_or_else(|| anyhow::anyhow!("model_providers.{provider_id} 必须�?TOML table"))
}
...
mod tests {
    ...
    fn relay_profile_model_prefers_config_then_field_then_empty() {
        // 1. 供應商測試的回退第一級：config.toml �?model = 優先
        ...
        // 2. config 沒寫 model 時退�?profile.model 欄位
        ...
        // 3. 兩者皆�?�?空字串；呼叫端據此才回退到全�?relayTestModel
