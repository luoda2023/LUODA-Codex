...
use luoda_codex_core::models::{DeleteResult, DeleteStatus, ExportResult, ExportStatus, SessionRef};
use luoda_codex_core::routes::{
    BridgeContext, BridgeDataService, BridgeRuntimeService, BridgeSettingsService,
    CoreRuntimeService, handle_bridge_request,
};
use luoda_codex_core::settings::BackendSettings;
...
async fn user_script_manager_deletes_market_script_metadata_and_rejects_builtin_delete() {
    ...
    let script = luoda_codex_core::script_market::MarketScript {
        id: "demo".to_string(),
        name: "Demo".to_string(),
    ...
    luoda_codex_core::script_market::install_market_script_content(
        &manager,
        &script,
        b"window.demo = true;",
    )
...
fn script_market_manifest_filters_invalid_entries() {
    ...
    let manifest = luoda_codex_core::script_market::parse_market_manifest(raw).unwrap();
...
fn user_script_inventory_includes_market_metadata() {
    ...
    manager
        .record_market_install(&luoda_codex_core::script_market::MarketScript {
            id: "demo".to_string(),
            name: "Demo".to_string(),
...
fn install_market_script_ignores_checksum_mismatch_and_replaces_existing_file() {
    ...
    luoda_codex_core::script_market::install_market_script_content(&manager, &script, b"new")
        .unwrap();
