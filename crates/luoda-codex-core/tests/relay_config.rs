...
fn apply_chat_protocol_relay_points_codex_to_local_responses_proxy() {
    ...
    let result = luoda_codex_core::relay_config::apply_relay_config_to_home_with_protocol(
        temp.path(),
    ...
    assert!(result.configured);
    assert!(updated.contains(r#"wire_api = "responses""#));
    assert!(updated.contains(r#"base_url = "http://127.0.0.1:56789/v1""#));
    assert!(updated.contains(r#"experimental_bearer_token = "sk-test-redacted""#));
...
"#
    ...
    assert!(
        profile
            .config_contents
            .contains(r#"base_url = "http://127.0.0.1:56789/v1""#)
    );
    ...
    assert!(live.contains(r#"base_url = "http://127.0.0.1:56789/v1""#));
}
...
fn apply_relay_files_with_context_selection_writes_all_global_context() {
    ...
    luoda_codex_core::relay_config::apply_relay_files_to_home_with_context(
        temp.path(),
        r#"model_provider = "custom"
[model_providers.custom]
name = "custom"
...
fn apply_relay_files_with_context_rejects_invalid_context_token_values() {
    ...
    let error = luoda_codex_core::relay_config::apply_relay_files_to_home_with_context(
        temp.path(),
        r#"model_provider = "custom""#,
        r#"{"OPENAI_API_KEY":"sk-new"}"#,
        "",
...
fn apply_relay_config_file_switches_config_without_touching_auth_json() {
    ...
    assert!(
        std::fs::read_to_string(home.join("config.toml"))
            .unwrap()
            .contains("http://127.0.0.1:56789/v1")
    );
    assert_eq!(
