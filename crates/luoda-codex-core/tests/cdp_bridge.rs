...
fn injection_script_prefixes_helper_url_and_sponsor_images() {
    ...
    assert!(script.contains(luoda_codex_core::version::VERSION));
...
fn injection_script_marks_diagnostic_build_and_reports_script_loaded() {
    ...
    assert!(script.contains(luoda_codex_core::assets::DIAGNOSTIC_BUILD_ID));
...
fn injection_script_fetches_ads_without_bridge() {
    ...
    assert!(script.contains("luoda2023/Ad-List"));
    assert!(
...
fn injection_script_explains_plugin_patch_is_unneeded_in_relay_mode() {
    ...
    assert!(script.contains("鍏煎澧炲己妯″紡涓嬫棤闇€寮€鍚?));
}
...
fn injection_script_restores_legacy_plugin_sidebar_entry_unlock() {
    ...
    assert!(script.contains("鎻掍欢 - 宸茶В閿?));
...
fn injection_script_keeps_bundled_marketplace_name_for_default_filter() {
    ...
    assert!(script.contains("if (name === \"openai-bundled\" || name === \"codex-plus-openai-bundled\") return \"OpenAI鎻掍欢1(Luoda-Codex)\""));
}
...
fn injection_script_expands_api_key_plugin_marketplace_requests() {
    ...
    assert!(script.contains("OpenAI鎻掍欢1(Luoda-Codex)"));
    assert!(script.contains("OpenAI鎻掍欢2(Luoda-Codex)"));
    assert!(script.contains("OpenAI鎻掍欢3(Luoda-Codex)"));
...
fn injection_script_exposes_conversation_view_width_control() {
    ...
    assert!(script.contains("瀵硅瘽灞呬腑瀹藉害"));
...
fn injection_script_moves_export_and_project_move_into_more_menu() {
    ...
    assert!(script.contains("configureActionButton(moreButton, \"鏇村鎿嶄綔\", \"鈥")"));
    assert!(script.contains("createSessionMoreMenuItem(\"瀵煎嚭\""));
    assert!(script.contains("createSessionMoreMenuItem(\"绉诲姩\""));
...
fn injection_script_does_not_add_delete_controls_on_archived_page() {
    ...
    assert!(!script.contains("鍒犻櫎鍏ㄩ儴褰掓。"));
}
...
fn injection_script_exposes_fast_service_tier_control() {
    ...
    assert!(script.contains("鍚庣鏈繛鎺ワ紝鏃犳硶鍒囨崲鏈嶅姟妯″紡"));
    assert!(script.contains("鏈繛鎺?));
    ...
    assert!(script.contains("缁ф壙 config.toml"));
    assert!(script.contains("service_tier=\\\"priority\\\""));
    assert!(script.contains("Fast 浠呮敮鎸?));
    assert!(script.contains("褰撳墠 thread"));
...
fn injection_script_prevents_switching_to_branches_used_by_other_worktrees() {
    ...
    assert!(script.contains("璇ュ垎鏀凡鍦ㄥ彟涓€涓?worktree 浣跨敤"));
...
fn manager_ui_exposes_pure_api_relay_mode_button() {
    let repo = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .expect("core crate should live under crates/luoda-codex-core");
    let source = std::fs::read_to_string(repo.join("apps/luoda-codex-manager/src/App.tsx")).unwrap();
    let commands =
        std::fs::read_to_string(repo.join("apps/luoda-codex-manager/src-tauri/src/lib.rs")).unwrap();

    assert!(source.contains("瀹樻柟娣峰叆 API Key"));
    assert!(source.contains("绾?API"));
...
async fn install_bridge_routes_binding_while_waiting_for_command_response() {
    ...
    luoda_codex_core::diagnostic_log::set_diagnostic_log_path_for_tests(None);
}
