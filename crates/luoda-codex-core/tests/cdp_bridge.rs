...
fn injection_script_prefixes_helper_url_and_sponsor_images() {
    ...
    assert!(script.contains("window.__luoda_codex_SPONSOR_IMAGES__"));
    assert!(script.contains("window.__luoda_codex_VERSION__"));
...
fn injection_script_marks_diagnostic_build_and_reports_script_loaded() {
    ...
    assert!(script.contains("window.__luoda_codex_BUILD__"));
...
window.__luoda_codex_TEST_SERVICE_TIER__ = true;
globalThis.document = {{
  scripts: [],
