...
fn watcher_install_plan_registers_rust_launcher_at_logon() {
    let plan = build_watcher_install_plan("C:/Tools/luoda-codex.exe".into(), 9333);

    assert_eq!(plan.run_value_name, "Luoda-CodexWatcher");
    assert_eq!(
        plan.run_value,
        "\"C:/Tools/luoda-codex.exe\" --debug-port 9333"
    );
    assert_eq!(plan.shortcut_name, "Luoda-CodexWatcher.lnk");
    assert_eq!(plan.shortcut_target, "C:/Tools/luoda-codex.exe");
    assert_eq!(plan.shortcut_arguments, "--debug-port 9333");
}

#[test]
fn spawn_launcher_command_points_to_silent_binary_only() {
    let command = build_spawn_launcher_command("C:/Tools/luoda-codex.exe", 9444);

    assert_eq!(command[0], "C:/Tools/luoda-codex.exe");
...
fn launcher_process_filter_protects_current_process_ancestry() {
    let processes = [
        (10, 0, "luoda-codex.exe"),
        (20, 10, "luoda-codex.exe"),
        (30, 20, "luoda-codex.exe"),
        (40, 10, "luoda-codex.exe"),
        (50, 10, "luoda-codex-manager.exe"),
    ];
