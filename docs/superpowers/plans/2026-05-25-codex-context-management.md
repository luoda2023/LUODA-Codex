...
- 修改 `crates/luoda-codex-core/src/settings.rs`
  - 为 `RelayProfile` 增加 `context_selection`、`context_window`、`auto_compact_limit`，全部带默认值。
- 修改 `crates/luoda-codex-core/src/relay_config.rs`
  - 增加 Codex 上下文 TOML 解析、upsert、delete、filter、上下文 token 字段写入。
  - 增加带上下文选择的完整文件切换函数。
- 修改 `crates/luoda-codex-core/tests/relay_config.rs`
  - 覆盖公共上下文解析、增删改、过滤、切换写入和数字校验。
- 修改 `apps/luoda-codex-manager/src-tauri/src/commands.rs`
  - 增加上下文管理命令和命令层测试。
  - 切换供应商时传入供应商上下文选择和上下文 token 字段。
- 修改 `apps/luoda-codex-manager/src-tauri/src/lib.rs`
  - 注册新增 Tauri commands。
- 修改 `apps/luoda-codex-manager/src/App.tsx`
  - 增加 TS 类型、默认值、normalize 兼容逻辑、供应商上下文 UI 和新增供应商默认全选逻辑。
- 修改 `apps/luoda-codex-manager/src/styles.css`
  - 增加上下文管理区布局和行样式。
...
**Files:**
- Modify: `crates/luoda-codex-core/src/settings.rs`
- Test: `crates/luoda-codex-core/src/settings.rs`
- Modify: `apps/luoda-codex-manager/src/App.tsx`

- [ ] **Step 1: 写 Rust 设置默认值失败测试**

在 `crates/luoda-codex-core/src/settings.rs` 的 `#[cfg(test)] mod tests` 中增加：
...
```powershell
cargo test -p luoda-codex-core settings::tests::relay_profile_context
```
...
在 `apps/luoda-codex-manager/src/App.tsx` 增加：
...
```powershell
git add crates/luoda-codex-core/src/settings.rs apps/luoda-codex-manager/src/App.tsx
...
**Files:**
- Modify: `crates/luoda-codex-core/src/relay_config.rs`
- Test: `crates/luoda-codex-core/tests/relay_config.rs`

- [ ] **Step 1: 写失败测试**

在 `crates/luoda-codex-core/tests/relay_config.rs` imports 增加：
...
```powershell
cargo test -p luoda-codex-core --test relay_config lists_codex_context_entries_from_common_config
```
...
```powershell
cargo test -p luoda-codex-core --test relay_config lists_codex_context_entries_from_common_config
cargo test -p luoda-codex-core --test relay_config upserts_and_deletes_context_entry_in_common_config
cargo test -p luoda-codex-core --test relay_config filters_common_config_for_supplier_selection
```
...
```powershell
git add crates/luoda-codex-core/src/relay_config.rs crates/luoda-codex-core/tests/relay_config.rs
...
在 `crates/luoda-codex-core/tests/relay_config.rs` 增加：
...
```powershell
cargo test -p luoda-codex-core --test relay_config apply_relay_files_with_context
```
...
**Files:**
- Modify: `apps/luoda-codex-manager/src-tauri/src/commands.rs`
- Modify: `apps/luoda-codex-manager/src-tauri/src/lib.rs`
- Test: `apps/luoda-codex-manager/src-tauri/src/commands.rs`
...
```powershell
cargo test -p luoda-codex-manager context_entry_commands_update_settings_payload
```
...
在 `apps/luoda-codex-manager/src-tauri/src/lib.rs` 的 `generate_handler!` 中加入：
...
```powershell
cargo test -p luoda-codex-manager context_entry_commands_update_settings_payload
cargo test -p luoda-codex-manager
```
...
```powershell
git add apps/luoda-codex-manager/src-tauri/src/commands.rs apps/luoda-codex-manager/src-tauri/src/lib.rs
...
**Files:**
- Modify: `apps/luoda-codex-manager/src/App.tsx`
- Modify: `apps/luoda-codex-manager/src/styles.css`
...
```powershell
git add apps/luoda-codex-manager/src/App.tsx apps/luoda-codex-manager/src/styles.css
...
```powershell
git add apps/luoda-codex-manager/src/App.tsx
...
Working directory: `apps/luoda-codex-manager`
...
```powershell
git add crates/luoda-codex-core/src/settings.rs crates/luoda-codex-core/src/relay_config.rs crates/luoda-codex-core/tests/relay_config.rs apps/luoda-codex-manager/src-tauri/src/commands.rs apps/luoda-codex-manager/src-tauri/src/lib.rs apps/luoda-codex-manager/src/App.tsx apps/luoda-codex-manager/src/styles.css
