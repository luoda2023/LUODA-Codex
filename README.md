...
<p align="center">
  <img src="docs/images/luoda-codex.png" alt="Luoda-Codex 图标" width="160">
</p>
...
Luoda-Codex 是面向 Codex App 的外部增强启动器和管理工具。它不修改 Codex App 原始安装文件，而是通过外部 launcher 启动 Codex，并使用 Chromium DevTools Protocol 注入增强脚本。

## 快速使用

从 [GitHub Releases](https://github.com/luoda2023/LUODA-Codex/releases) 下载最新版安装包：
...
- `Luoda-Codex`：静默启动入口，不显示管理界面，只负责启动 Codex 并注入增强功能。
- `Luoda-Codex 管理工具`：Tauri 控制面板，用于启动、检查、修复、更新、配置中转注入、管理增强功能和用户脚本。

Windows 安装包会创建桌面和开始菜单快捷方式。macOS DMG 会安装 `/Applications/Luoda-Codex.app` 和 `/Applications/Luoda-Codex 管理工具.app`。
...
<p align="center">
  <a href="https://jojocode.com/"><strong>JOJO Code｜Luoda-Codex 官方中转站</strong></a><br>
  Luoda-Codex 官方中转站，主打稳定接入和划算价格，支持 GPT-5.5、GPT-5.4、Claude Opus 4.8、Claude Opus 4.7、gpt-image-2 等模型与图像能力，适合日常开发、团队协作和长期项目工作流。
</p>
...
<table>
  ...
  <tr>
    ...
    <td><a href="https://jojocode.com/"><strong>JOJO Code｜Luoda-Codex 官方中转站</strong></a><br>感谢 JOJO Code 赞助本项目。JOJO Code 是 Luoda-Codex 官方中转站，提供价格划算、稳定易接入的 Codex API 中转服务，支持 GPT-5.5、GPT-5.4、Claude Opus 4.8、Claude Opus 4.7、gpt-image-2 等模型与图像能力，适合日常开发、快速配置、团队协作和长期使用。</td>
  </tr>
  <tr>
    <td align="center">
    ...
    <td><a href="https://aigocode.com/invite/LUODA-Codex"><strong>AIGoCode</strong></a><br>感谢 AIGoCode 赞助了本项目！AIGoCode 是一个集成了 Claude Code、Codex 以及 Gemini 最新模型的一站式平台，为你提供稳定、高效且高性价比的AI编程服务。本站提供灵活的订阅计划，支持多风险，国内直连，无需魔法，极速响应。AIGoCode 为 LUODA-Codex 的用户提供了特别福利，通过<a href="https://aigocode.com/invite/LUODA-Codex">此链接注册</a>的用户首次充值可以获得额外10%奖励额度！</td>
  </tr>
  <tr>
    <td align="center">
    ...
    <td><a href="https://www.packyapi.com/"><strong>PackyCode</strong></a><br>感谢 PackyCode 赞助了本项目！PackyCode 是一家稳定、高效的API中转服务商，提供 Claude Code、Codex、Gemini 等多种中转服务。PackyCode 为本软件的用户提供了特别优惠，使用此链接注册并在充值时填写"LUODA-Codex"优惠码，首次充值可以享受9折优惠！</td>
  </tr>
  <tr>
    <td align="center">
...
欢迎加入 Luoda-Codex 交流群（QQ群：1103050832），反馈问题、交流使用体验或提出新功能建议。

微信群：<a href="https://docs.qq.com/doc/DQ2VOanZTTFZJcUpZ#">点击这里获取最新微信群二维码</a>。

<img src="docs/images/discussion-group-qr.jpg" alt="Luoda-Codex 微信群二维码" width="260">

Telegram 频道：<https://t.me/LUODA-Codex>

如果 Luoda-Codex 帮到了你，可以请我喝杯咖啡，或者随手赞赏支持一下继续维护。
...
- 中转注入模式：支持多个中转配置，写入 `LUODA-Codex` provider，并可切回官方 ChatGPT 登录态。
...
Luoda-Codex 启动后会解锁插件入口，并在会话列表悬停时显示删除按钮：

![Luoda-Codex 解锁插件入口并添加删除按钮](docs/images/solution-plugin-and-delete.png)

顶部菜单栏会出现 `Luoda-Codex`，可以查看后端状态并打开设置面板：

![Luoda-Codex 后端状态指示灯](docs/images/backend-status-indicator.png)
![Luoda-Codex 设置面板](docs/images/settings-panel.png)
...
Luoda-Codex 会在 `~/.codex/config.toml` 中写入类似配置：
...
增强功能在管理工具中统一开关。默认开启增强注入；关闭后不会注入 Luoda-Codex 菜单和脚本。
...
Luoda-Codex 通过 GitHub Release 发布安装包。Windows 会生成 NSIS 安装程序，macOS 会生成 Intel x64 和 Apple Silicon arm64 两个 DMG。
...
- Luoda-Codex 状态与日志：`~/.codex-session-delete/`
...
确认是从 `Luoda-Codex` 入口启动，而不是原版 Codex。也可以打开管理工具的“诊断”和“日志”页面查看注入状态。
...
如果接口正常，但插件仍显示超时，通常是 Codex 页面里的 CDP bridge 或脚本缓存问题。重启 Luoda-Codex，或在管理工具里查看日志中的 `renderer.script_loaded`、`bridge.request`、`bridge.response`。

### Upstream worktree 和 Codex 原生创建有什么区别

Luoda-Codex 的 Upstream worktree 功能等价于先更新远端分支，再执行：

```bash
git worktree add -b <new-branch> <worktree-path> upstream/<base-branch>
```

这样新 worktree 从最新的远端跟踪分支开始，而不是从当前会话所在的本地 HEAD 开始。如果 Luoda-Codex 无法安全识别当前 Codex 版本的原生 worktree 创建表单，请从 Luoda-Codex 菜单中手动填写仓库路径、分支名、worktree 路径、remote 和 base branch。

### macOS 提示无法打开或已损坏

当前安装包未签名/未公证时，macOS Gatekeeper 可能拦截，出现“已损坏，无法打开”的提示：

![macOS 提示 Luoda-Codex 管理工具已损坏](docs/images/macos-damaged-warning.png)

如果遇到该提示，可以在终端执行下面两条命令，解除苹果系统的安全隔离限制：

```bash
sudo xattr -rd com.apple.quarantine /Applications/Luoda-Codex\ 管理工具.app
sudo xattr -rd com.apple.quarantine /Applications/Luoda-Codex.app
```

执行后重新打开 `Luoda-Codex` 或 `Luoda-Codex 管理工具` 即可。
...
scripts/installer/
  windows/LUODA-Codex.nsi     Windows NSIS 安装包
...
Luoda-Codex 是外部增强工具，不修改 Codex App 原始文件。Codex App 更新后，如果页面结构变化，可能需要更新注入脚本。
