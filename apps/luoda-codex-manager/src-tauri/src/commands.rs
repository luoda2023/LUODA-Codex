...
#[tauri::command]
pub fn get_config_coordination_status() -> CommandResult<luoda_codex_core::config_coordinator::CoordinationStatus> {
    let settings =
...
pub fn list_local_sessions() -> CommandResult<LocalSessionsPayload> {
    let db_path = luoda_codex_core::relay_config::default_codex_home_dir().join("state_5.sqlite");
...
pub async fn load_provider_sync_targets() -> CommandResult<Value> {
    ...
    let result = tauri::async_runtime::spawn_blocking(|| {
        luoda_codex_data::load_provider_sync_targets(None)
    })
    .await
