...
use luoda_codex_core::install::SILENT_BINARY;
use luoda_codex_core::models::{DeleteResult, SessionRef};
use luoda_codex_core::script_market::{self, MarketScript, ScriptMarketManifest};
use luoda_codex_core::settings::{BackendSettings, RelayProfile, SettingsStore};
use luoda_codex_core::status::{LaunchStatus, StatusStore};
use luoda_codex_core::user_scripts::UserScriptManager;
use luoda_codex_core::zed_remote::{ZedOpenStrategy, ZedRemoteProject};
use serde::Serialize;
...
pub struct LocalSessionsPayload {
    ...
    pub sessions: Vec<luoda_codex_data::LocalSession>,
}
...
pub struct CcsProvidersPayload {
    pub db_path: String,
    pub providers: Vec<luoda_codex_core::ccs_import::CcsProviderImport>,
}
...
pub struct ContextEntriesPayload {
    pub settings: BackendSettings,
    pub entries: luoda_codex_core::relay_config::CodexContextEntries,
}
...
pub fn backend_version() -> CommandResult<VersionPayload> {
    ok(
        "后端版本已读取�?,
        VersionPayload {
            version: luoda_codex_core::version::VERSION.to_string(),
        },
    )
}
...
pub fn startup_should_show_update() -> bool {
    should_show_update(
        std::env::args(),
        std::env::var("luoda_codex_SHOW_UPDATE").ok().as_deref(),
    )
}
    ...
    let Ok((codex_app_path, entrypoints, latest_launch)) = payload else {
        return failed(
            ...
            OverviewPayload {
                ...
                latest_launch: None,
                current_version: luoda_codex_core::version::VERSION.to_string(),
                update_status: "not_checked".to_string(),
                settings_path: luoda_codex_core::paths::default_settings_path()
                    .to_string_lossy()
                    .to_string(),
                logs_path: luoda_codex_core::paths::default_diagnostic_log_path()
                    .to_string_lossy()
                    .to_string(),
            },
        );
    ...
    ok(
        ...
        OverviewPayload {
            codex_version: codex_app_path
                .as_deref()
                .and_then(luoda_codex_core::app_paths::codex_app_version),
...
pub fn launch_codex_plus(request: LaunchRequest) -> CommandResult<Value> {
    spawn_luoda_codex_launch(request, "启动任务已在后台开始，可稍后查看概览状态�?)
}

#[tauri::command]
pub fn restart_codex_plus(request: LaunchRequest) -> CommandResult<Value> {
    luoda_codex_core::watcher::stop_launcher_processes();
    luoda_codex_core::watcher::stop_codex_processes();
    spawn_luoda_codex_launch(request, "Codex 已请求重启，启动任务正在后台运行�?)
}

fn spawn_luoda_codex_launch(request: LaunchRequest, accepted_message: &str) -> CommandResult<Value> {
...
fn spawn_silent_launcher(request: &LaunchRequest) -> anyhow::Result<()> {
    let launcher = luoda_codex_core::install::companion_binary_path(SILENT_BINARY);
...
-> CommandResult<luoda_codex_core::config_coordinator::CoordinationStatus> {
    let settings =
        settings_with_live_ccs_profiles(SettingsStore::default().load().unwrap_or_default());
    ok(
        "已读取配置协调状态�?,
        luoda_codex_core::config_coordinator::coordination_status(&settings),
    )
}
...
pub fn save_settings(settings: BackendSettings) -> CommandResult<SettingsPayload> {
    ...
    if settings.ccs_link_enabled {
        if let Err(error) = luoda_codex_core::ccs_import::write_linked_profiles_to_default_db(
        ...
        if !active.linked_ccs_provider_id.trim().is_empty() {
            if let Err(error) =
                luoda_codex_core::ccs_import::set_current_codex_provider_in_default_db(
...
pub fn load_ccs_providers() -> CommandResult<CcsProvidersPayload> {
    let db_path = luoda_codex_core::ccs_import::default_ccs_db_path();
    match luoda_codex_core::ccs_import::list_codex_providers_from_db(&db_path) {
        Ok(providers) => ok(
            &format!("已读取外�?Codex 供应商配置：{} 个�?, providers.len()),
            CcsProvidersPayload {
...
pub fn import_ccs_providers() -> CommandResult<SettingsPayload> {
    ...
    let synced = match luoda_codex_core::ccs_import::list_codex_providers_from_default_db() {
    ...
    match store.save(&settings) {
        Ok(()) => settings_payload(
            &format!("已开�?cc-switch 联动：{synced} 个供应商将直接从 cc-switch 读取�?),
            "联动供应商配置后重新读取设置失败",
        ),
        Err(error) => failed(
...
pub fn list_local_sessions() -> CommandResult<LocalSessionsPayload> {
    let home = luoda_codex_core::codex_sqlite::default_codex_home_dir();
    let db_paths = luoda_codex_core::codex_sqlite::codex_session_db_paths_from_home(&home);
    ...
    if errors.is_empty() {
        ok(
            &format!("已读�?{} 个本地会话�?, payload.sessions.len()),
            payload,
        )
    } else {
        failed(
...
pub fn list_zed_remote_projects() -> CommandResult<ZedRemoteProjectsPayload> {
    let result = luoda_codex_core::zed_remote::list_zed_remote_projects_response(&json!({}));
    if result.get("status").and_then(Value::as_str) == Some("ok") {
        ...
        .unwrap_or_default();
        return ok(
            &format!("已读�?{} �?Zed 远程项目�?, projects.len()),
...
pub fn open_zed_remote(payload: Value) -> CommandResult<ZedRemoteOpenPayload> {
    let result = luoda_codex_core::zed_remote::open_zed_remote(&payload);
    let strategy = result
        .get("strategy")
        .cloned()
...
pub fn forget_zed_remote_project(id: String) -> CommandResult<ZedRemoteProjectsPayload> {
    let result =
        luoda_codex_core::zed_remote::forget_zed_remote_project_response(&json!({ "id": id }));
    if result.get("status").and_then(Value::as_str) != Some("ok") {
        return failed(
            result
                .get("message")
...
pub fn delete_local_session(request: DeleteLocalSessionRequest) -> CommandResult<DeleteResult> {
    ...
    if session_id.is_empty() {
        return failed(
            "会话 ID 不能为空�?,
            DeleteResult {
                status: luoda_codex_core::models::DeleteStatus::Failed,
    ...
    let candidate_paths = request
        ...
        .unwrap_or_else(|| {
            luoda_codex_core::codex_sqlite::codex_session_db_paths_from_home(
                &luoda_codex_core::codex_sqlite::default_codex_home_dir(),
            )
        });
    ...
    for db_path in candidate_paths {
        ...
        if matches!(
            result.status,
            luoda_codex_core::models::DeleteStatus::LocalDeleted
        ) {
            break;
        }
    }
...
fn local_session_adapter(db_path: &Path) -> luoda_codex_data::SQLiteStorageAdapter {
    luoda_codex_data::SQLiteStorageAdapter::new(
        db_path,
        luoda_codex_data::BackupStore::new(
            luoda_codex_core::paths::default_app_state_dir().join("backups"),
        ),
    )
}
...
fn normalize_settings_before_save(mut settings: BackendSettings) -> BackendSettings {
    if let Some(path) =
        luoda_codex_core::app_paths::normalize_codex_app_path(Path::new(&settings.codex_app_path))
    {
        settings.codex_app_path = path.to_string_lossy().to_string();
    }
    settings.relay_common_config_contents =
        luoda_codex_core::relay_config::sanitize_common_config_contents(
            &settings.relay_common_config_contents,
        );
    ...
    for profile in &mut settings.relay_profiles {
        if let Err(error) =
            luoda_codex_core::relay_config::normalize_relay_profile_for_storage(profile)
        {
            log_manager_event(
    ...
    if !common_config.trim().is_empty() {
        for profile in &mut settings.relay_profiles {
            ...
            match luoda_codex_core::relay_config::strip_common_config_from_config(
...
fn settings_with_live_ccs_profiles(mut settings: BackendSettings) -> BackendSettings {
    ...
    remove_linked_ccs_profiles_for_local_storage(&mut settings);
    if let Err(error) = luoda_codex_core::ccs_import::sync_linked_profiles_from_default_db(
...
fn remove_linked_ccs_profiles_for_local_storage(settings: &mut BackendSettings) {
            ...
            .any(|profile| profile.id == settings.active_relay_id)
    {
        settings.active_relay_id = settings
            ...
            .unwrap_or_else(luoda_codex_core::settings::default_active_relay_id);
    }
}
...
fn relay_join_config_sections(sections: &[&str]) -> String {
    ...
    if sections.is_empty() {
        ...
        luoda_codex_core::relay_config::normalize_config_text(&format!(
            "{}\n",
            sections.join("\n\n")
        ))
    }
...
pub async fn load_provider_sync_targets() -> CommandResult<Value> {
    ...
    let result =
        tauri::async_runtime::spawn_blocking(|| luoda_codex_data::load_provider_sync_targets(None))
            .await
...
fn merge_manual_provider_sync_targets(
    targets: &mut luoda_codex_data::ProviderSyncTargetList,
    manual: &[String],
    ...
    for id in manual {
        if let Some(existing) = targets.targets.iter_mut().find(|target| target.id == *id) {
            if !existing
                .sources
                .contains(&luoda_codex_data::ProviderSyncTargetSource::Manual)
            {
                existing
                    .sources
                    .push(luoda_codex_data::ProviderSyncTargetSource::Manual);
            ...
            targets
                .targets
                .push(luoda_codex_data::ProviderSyncTargetOption {
                    id: id.clone(),
                    sources: vec![luoda_codex_data::ProviderSyncTargetSource::Manual],
...
pub async fn sync_providers_now(target_provider: Option<String>) -> CommandResult<Value> {
    ...
    let result = tauri::async_runtime::spawn_blocking(move || {
        luoda_codex_data::run_provider_sync_with_target(None, target_provider.as_deref())
    })
    .await
...
fn is_success_sync_status(status: &luoda_codex_data::ProviderSyncStatus) -> bool {
    matches!(status, luoda_codex_data::ProviderSyncStatus::Synced)
}
...
pub async fn load_ads() -> CommandResult<AdsPayload> {
    match luoda_codex_core::ads::fetch_ad_list().await {
...
pub fn open_external_url(url: String) -> CommandResult<Value> {
    ...
    if !(trimmed.starts_with("https://") || trimmed.starts_with("http://")) {
        return failed("只允许打开 http �?https 链接�?, json!({}));
    }
...
pub fn repair_backend() -> CommandResult<SettingsPayload> {
    let settings =
        settings_with_live_ccs_profiles(SettingsStore::default().load().unwrap_or_default());
    let message = match luoda_codex_core::cli_wrapper::ensure_cli_wrapper(&settings) {
    ...
    settings_payload(&message, "修复后重新读取设置失�?)
}

#[tauri::command]
pub async fn check_update() -> CommandResult<Value> {
    match luoda_codex_core::update::check_for_update(luoda_codex_core::version::VERSION).await {
        Ok(update) => {
        ...
        Err(error) => failed(
            &format!("检查更新失败：{error}"),
            json!({
                "currentVersion": luoda_codex_core::version::VERSION,
...
pub async fn perform_update(
    release: Option<luoda_codex_core::update::Release>,
) -> CommandResult<Value> {
    ...
    let download_dir = luoda_codex_core::paths::default_app_state_dir().join("updates");
    match luoda_codex_core::update::perform_update(&release, &download_dir).await {
        Ok(result) => ok(
...
pub fn install_watcher() -> CommandResult<WatcherPayload> {
    let launcher_path =
        luoda_codex_core::install::companion_binary_path(luoda_codex_core::install::SILENT_BINARY);
    match luoda_codex_core::watcher::install_watcher(&launcher_path, default_debug_port()) {
...
pub fn uninstall_watcher() -> CommandResult<WatcherPayload> {
    match luoda_codex_core::watcher::uninstall_watcher() {
...
pub fn enable_watcher() -> CommandResult<WatcherPayload> {
    match luoda_codex_core::watcher::enable_watcher() {
...
pub fn disable_watcher() -> CommandResult<WatcherPayload> {
    match luoda_codex_core::watcher::disable_watcher() {
...
pub fn read_latest_logs(request: LogRequest) -> CommandResult<LogsPayload> {
    let path = luoda_codex_core::paths::default_diagnostic_log_path();
...
pub fn reset_settings() -> CommandResult<SettingsPayload> {
    ...
    match SettingsStore::default().save(&settings) {
        Ok(()) => settings_payload("设置已重置为默认值�?, "设置重置后重新读取失�?),
        Err(error) => failed(
            &format!("重置设置失败：{error}"),
            SettingsPayload {
                settings,
...
pub fn relay_status() -> CommandResult<RelayPayload> {
    let status = luoda_codex_core::relay_config::default_relay_status();
    let message = if status.authenticated {
        "已检测到 ChatGPT 登录状态�?
    } else {
        "未检测到 ChatGPT 登录状态，请先�?Codex/ChatGPT 中正常登录�?
    };
...
pub fn read_relay_files() -> CommandResult<RelayFilesPayload> {
    let home = luoda_codex_core::relay_config::default_codex_home_dir();
...
pub fn write_diagnostic_event(event: String, detail: Value) -> CommandResult<Value> {
    ...
    match luoda_codex_core::diagnostic_log::append_diagnostic_log(&event, detail) {
        Ok(()) => ok("诊断日志已写入�?, json!({})),
...
pub fn backfill_relay_profile_from_live(
    ...
    match luoda_codex_core::relay_config::backfill_relay_profile_from_home_with_common(
        &home,
        profile,
...
pub fn list_context_entries(
    ...
    match luoda_codex_core::relay_config::list_context_entries_from_common_config(
...
pub fn read_live_context_entries() -> CommandResult<LiveContextEntriesPayload> {
    ...
    match luoda_codex_core::relay_config::list_context_entries_from_common_config(&config) {
        Ok(entries) => ok(
            "live 工具与插件已读取�?,
...
pub fn upsert_context_entry(request: ContextEntryRequest) -> CommandResult<ContextEntriesPayload> {
    let mut settings = request.settings;
    match luoda_codex_core::relay_config::upsert_context_entry_in_common_config(
        &settings.relay_context_config_contents,
        &request.kind,
        &request.id,
        &request.toml_body,
...
pub fn sync_live_context_entries(
    ...
    let updated_config = match luoda_codex_core::relay_config::sync_live_config_context_entries(
        &current_config,
        &request.settings.relay_context_config_contents,
    ) {
        Ok(config) => config,
    ...
    match luoda_codex_core::relay_config::list_context_entries_from_common_config(&updated_config) {
        Ok(entries) => ok(
            "live 工具与插件已同步�?,
            LiveContextEntriesPayload { entries },
        ),
...
pub fn delete_context_entry(request: ContextDeleteRequest) -> CommandResult<ContextEntriesPayload> {
    let mut settings = request.settings;
    match luoda_codex_core::relay_config::delete_context_entry_from_common_config(
        &settings.relay_context_config_contents,
        &request.kind,
        &request.id,
    ) {
...
pub fn extract_relay_common_config(
    ...
    match luoda_codex_core::relay_config::extract_common_config_from_config(&request.config_contents)
        .and_then(|common_config_contents| {
            let profile_config_contents =
                luoda_codex_core::relay_config::strip_common_config_from_config(
...
pub async fn test_relay_profile(profile: RelayProfile) -> CommandResult<RelayProfileTestPayload> {
    ...
    let test_model: String = if !profile.test_model.trim().is_empty() {
        ...
        let from_profile = luoda_codex_core::relay_config::relay_profile_model(&profile);
    ...
    match luoda_codex_core::relay_config::test_relay_profile(&profile, &test_model).await {
        Ok(result) => {
            ...
            CommandResult {
                status: status.to_string(),
                message: format!(
                    "已向「{profile_name}」用模型「{test_model}」发�?hi，HTTP {}。{detail}",
                    result.http_status
                ),
...
pub async fn fetch_relay_profile_models(
    ...
    match luoda_codex_core::model_catalog::fetch_relay_profile_model_ids(&profile).await {
        Ok((models, endpoint)) => ok(
            &format!("已从「{profile_name}」获�?{} 个模型�?, models.len()),
...
pub fn apply_relay_injection() -> CommandResult<RelayPayload> {
    ...
    if !settings.relay_profiles_enabled {
        let status = luoda_codex_core::relay_config::relay_status_from_home(&home);
        return failed(
    ...
    if relay_has_complete_files(&relay) {
        return match luoda_codex_core::relay_config::apply_relay_profile_to_home_with_switch_rules(
            &home,
            &relay,
    ...
    let auth = luoda_codex_core::relay_config::chatgpt_auth_status_from_home(&home);
    if !auth.authenticated {
        ...
        log_relay_apply_result(
            ...
            &status,
            None,
            Some("未检测到 ChatGPT 登录状�?.to_string()),
        );
        return failed(
    ...
    match luoda_codex_core::relay_config::apply_relay_config_to_home_with_protocol(
        &home,
        &relay.base_url,
        &relay.api_key,
        relay.protocol,
        luoda_codex_core::protocol_proxy::DEFAULT_PROTOCOL_PROXY_PORT,
    ) {
        Ok(result) => {
            let status = luoda_codex_core::relay_config::relay_status_from_home(&home);
            log_relay_apply_result(
...
pub fn apply_pure_api_injection() -> CommandResult<RelayPayload> {
        ...
        return match luoda_codex_core::relay_config::apply_relay_profile_to_home_with_switch_rules(
            ...
            Ok(result) => {
                ...
                if !status.configured {
                    return failed(
                        "�?API 配置写入后未检测到完整 custom provider，请检�?config.toml 和供应商 API Key�?,
                        relay_payload(status, result.backup_path),
                    );
                }
                ok(
    ...
    match luoda_codex_core::relay_config::apply_pure_api_config_to_home_with_protocol(
        &home,
        &relay.base_url,
        &relay.api_key,
        relay.protocol,
        ...
        Ok(result) => {
            ...
            ok(
                "�?API 模式已写入：config.toml 已写�?custom provider，auth.json 已切换为当前供应商�?,
                relay_payload(status, result.backup_path),
            )
        }
        Err(error) => {
...
pub fn clear_relay_injection() -> CommandResult<RelayPayload> {
    ...
    let auth_contents = (relay.relay_mode == luoda_codex_core::settings::RelayMode::Official
        && !relay.official_mix_api_key
        && !relay.auth_contents.trim().is_empty())
    .then_some(relay.auth_contents.as_str());
    match luoda_codex_core::relay_config::clear_relay_config_to_home_with_auth(&home, auth_contents)
    {
        Ok(result) => {
            let status = luoda_codex_core::relay_config::relay_status_from_home(&home);
            log_manager_event(
            ...
            ok(
                "已清�?custom 中转 API 模式，并切换到官�?ChatGPT 登录模式�?,
                relay_payload(status, result.backup_path),
            )
        }
        Err(error) => {
...
fn try_apply_linked_ccs_provider(
    settings: &BackendSettings,
    relay: &luoda_codex_core::settings::RelayProfile,
) -> Option<CommandResult<RelayPayload>> {
    if !settings.ccs_link_enabled {
        return None;
    }
    if luoda_codex_core::config_coordinator::effective_ownership(settings)
        != luoda_codex_core::settings::ConfigOwnership::CcSwitch
    {
        return None;
    }
    ...
    let home = luoda_codex_core::relay_config::default_codex_home_dir();
    match luoda_codex_core::config_coordinator::apply_linked_ccs_provider_to_home(
        source_id,
        &relay_combined_common_config(settings),
    ) {
        Ok(result) => {
            ...
            Some(ok(
                "已通过 cc-switch 联动应用当前供应商配置，避免�?CC Switch 发生覆盖冲突�?,
                relay_payload(status, result.backup_path),
            ))
        }
        Err(error) => {
...
fn guard_live_write_or_fail(
    ...
    let decision = luoda_codex_core::config_coordinator::evaluate_live_write(settings, force);
    if decision.allowed {
        return None;
    }
    let home = luoda_codex_core::relay_config::default_codex_home_dir();
...
fn relay_has_complete_files(relay: &luoda_codex_core::settings::RelayProfile) -> bool {
    if relay.relay_mode == luoda_codex_core::settings::RelayMode::Official
        && relay.official_mix_api_key
    {
...
fn log_relay_apply_result(
    event: &str,
    relay: &luoda_codex_core::settings::RelayProfile,
    status: &luoda_codex_core::relay_config::RelayStatus,
...
fn log_manager_event(event: &str, detail: Value) {
    let _ = luoda_codex_core::diagnostic_log::append_diagnostic_log(event, detail);
}
...
fn refresh_cli_wrapper_after_settings_save(settings: &BackendSettings) -> String {
    match luoda_codex_core::cli_wrapper::ensure_cli_wrapper(settings) {
        Ok(Some(install)) => format!(
            " 命令包装器已更新：{}�?,
            install.real_codex.to_string_lossy()
        ),
...
fn empty_context_entries() -> luoda_codex_core::relay_config::CodexContextEntries {
    luoda_codex_core::relay_config::CodexContextEntries {
        mcp_servers: Vec::new(),
        skills: Vec::new(),
        plugins: Vec::new(),
    }
...
fn settings_payload_value() -> Result<SettingsPayload, (anyhow::Error, SettingsPayload)> {
    let store = SettingsStore::default();
    let settings_path = luoda_codex_core::paths::default_settings_path()
        .to_string_lossy()
        .to_string();
    match store.load() {
...
fn diagnostics_report() -> String {
    ...
    serde_json::to_string_pretty(&json!({
        ...
        "version": luoda_codex_core::version::VERSION,
        "overview": overview.payload,
        "settings": settings,
        "logs": {
            "diagnosticLogPath": luoda_codex_core::paths::default_diagnostic_log_path(),
            "latestStatusPath": luoda_codex_core::paths::default_latest_status_path()
        },
        "platform": {
...
fn load_overview_payload() -> (
    ...
    let settings = SettingsStore::default().load().unwrap_or_default();
    (
        luoda_codex_core::app_paths::resolve_codex_app_dir_with_saved(
            None,
...
fn watcher_payload() -> WatcherPayload {
    let flag = luoda_codex_core::watcher::default_watcher_disabled_flag();
    WatcherPayload {
        enabled: !flag.exists(),
...
mod tests {
    ...
    fn startup_options_honors_show_update_environment() {
        ...
        unsafe {
            std::env::remove_var("luoda_codex_SHOW_UPDATE");
        }
    ...
    fn update_install_requires_release_payload() {
        ...
        assert!(result.message.contains("请先检查更�?));
    }
    ...
    fn relay_payload_does_not_expose_token_text() {
        let payload = relay_payload(
            luoda_codex_core::relay_config::RelayStatus {
                authenticated: true,
    ...
    fn apply_relay_profile_to_home_with_switch_rules_preserves_custom_provider_id() {
        ...
        let profile = RelayProfile {
            relay_mode: luoda_codex_core::settings::RelayMode::PureApi,
            protocol: luoda_codex_core::settings::RelayProtocol::Responses,
        ...
        luoda_codex_core::relay_config::apply_relay_profile_to_home_with_switch_rules(
            temp.path(),
            &profile,
            "",
        )
    ...
    fn normalize_settings_before_save_preserves_official_profile_auth() {
        let settings = BackendSettings {
            relay_profiles: vec![RelayProfile {
                relay_mode: luoda_codex_core::settings::RelayMode::Official,
...
"#
    ...
    fn open_external_url_rejects_non_http_urls() {
        ...
        assert_eq!(result.status, "failed");
        assert!(result.message.contains("只允许打开 http �?https 链接"));
    }
}
